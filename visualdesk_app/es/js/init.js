...
Parse.initialize("iWxoRM3p3NMvWbuCOmLhMN7eHJKo5a12oTsRV0K2", "MLEzE5JYOojmLslNvE5T8WpvuatiDUUrcfQWqSjM");
...
$(function () { // DOM ready
  ...
  if (platform == "Sugar" || platform == "Web") {
    var player_name = "A " + platform + " Player";
  } else {
    if (platform == "Android") {
      var player_name = "An Android Player";
    }
  }
  ...
  $("#hs_button").click(function() {
    ...
    var highscores = "Las mejores 10 puntuaciones:\n\n"
    query.descending("score");
    query.limit(10);
    query.find({
      success: function(results) {
        if (debug_mode) alert("Successfully retrieved " + results.length + " scores.");
        // Do something with the returned Parse.Object values
        for (var i = 0; i < results.length; i++) { 
          var object = results[i];
          highscores = highscores + object.get('score') + " - " + object.get('playerName') + "\n";
        }
        alert(highscores);
      },
  ...
  $("#name_button").click(function(){
    player_name_prompt = prompt("Como es tu nombre completo?","");

    var GameScore = Parse.Object.extend("GameScore");
    var gameScore = new GameScore();
     
    gameScore.set("score", parseInt($("#current_score_num").html()));
    gameScore.set("cheatMode", hint_mode);
    
    if (player_name_prompt.length == 0) {
      console.log("The player doesn't typed her/his name, sending the default name.")
      gameScore.set("playerName", player_name);
    } else {
      gameScore.set("playerName", player_name_prompt);
    }
    ...
    gameScore.save(null, {
      success: function(gameScore) {
        ...
        if (debug_mode) {
          ...
          alert("Ya hemos enviado tu puntuación! :-)");
        }
      },
      error: function(gameScore, error) {
        ...
        if (debug_mode) {
          ...
          alert("Uh, hemos tenido un error... Verifica tu conexión a internet e intentalo otra vez...");
        }
      }
    });
  });  
  ...
  $('.bt_new_game').live('click', function () {
    ...
    _gaq.push(['_trackEvent', 'Diamond Fusion', 'Play again', 'Play again']);
    init_game();
  });
...
function init_game() {
  ...
  // tracking Google Analytics
  _gaq.push(['_trackEvent', 'Diamond Fusion', 'Game start', 'Game start']);
...
function verif_tableau() {
  ...
  if ($icone != undefined && $binome != undefined) {
    if ($icone.hasClass('hypercube') && !$icone.hasClass('new')) {
      ...
      $('#zone_message').append('<div class="hypercube">EXCELENTE!</div>');
  ...
  if (chaine_trouvee) {
    ...
    if ($icone == undefined && $binome == undefined) {
      if (test_possible_move()) {
        ...
        // tracking Google Analytics
        _gaq.push(['_trackEvent', 'Diamond Fusion', 'Fin del juego', 'Fin del juego', score]);
        $('#zone_message').html('<div class="bad">Fin del juego</div>');
        $('#zone_message').append('<div class="good">' + score + ' puntos</div>');
        if (score > best_score) {
          best_score = score;
        ...
        $('#zone_message').append('<script>!function(d,s,id){var js,fjs=d.getElementsByTagName(s)[0];if(!d.getElementById(id)){js=d.createElement(s);js.id=id;js.src="https://platform.twitter.com/widgets.js";fjs.parentNode.insertBefore(js,fjs);}}(document,"script","twitter-wjs");</script><a href="https://twitter.com/share" class="twitter-share-button" data-text="I scored ' + score + ' on Diamond Fusion! Can you beat me? #diamondfusion" data-url="http://diamondfusion.tk/" data-counturl="http://www.diamondfusion.tk/" data-lang="en" data-count="vertical">Tweet your score</a>');
        $('#zone_message').append('<div class="fb-share-button" data-href="http://diamondfusion.tk/" data-text="I scored ' + score + ' on Diamond Fusion! Can you beat me? #diamondfusion" data-type="box_count"></div>');
        if (combo > best_combo) {
          best_combo = combo;
...
function test_chaine(ligne, colonne) {
  ...
  if (!test_verti[ligne][colonne]) {
    ...
    if (suite_verti >= 3) {
      ...
      if(suite_verti == 4){
        // animate BOLA DE FUEGO creation
      ...
      while(ligne+i < NB_COLONNES && tab_icones[ligne+i][colonne] == num_icone) {
        ...
        if(i == 1 && multiplier%5 == 0){
          ...
          if(i == 1 && suite_verti == 4){
            ...
            $('#zone_message').append('<div class="fire">BOLA DE FUEGO!</div>');
          } else {
...
function explosion(ligne, colonne) {
  ...
  $('#zone_message').append('<div class="fire">GENIAL!</div>');
  setTimeout(function () {
    $explosion.remove();
  }, 600);
...
function destroy_line_column(ligne, colonne) {
  $('#zone_message').append('<div class="star">GENIAL!</div>');
  for (var i = 0; i < NB_LIGNES; i++) {
...
function test_possible_move() {
  ...
  if(nb_possible_moves <= 3) {
    if(nb_possible_moves <= 1) {
      $('#moves').addClass('critical').html(nb_possible_moves + '<br>possible move');
    } else {
      $('#moves').removeClass('critical').html(nb_possible_moves + '<br>possible moves');
    }
  } else {
